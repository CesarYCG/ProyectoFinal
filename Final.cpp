...
int main()
{
	...
	// Modelo Aldair
	Model sueloC1("resources/objects/C1/room.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// DIBUJANOD CUARTO LIVIANO
		model = glm::translate(glm::mat4(1.0f), glm::vec3(128.0, -1.0f, -240.0f)); // Trasladamos el objeto a una posicion
		model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.25f));
		staticShader.setMat4("model", model); // Utilizamos los valores del static shader
		sueloC1.Draw(staticShader);
